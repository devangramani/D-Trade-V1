...
function FutCard(props) {
  ...
  const _delete_script = item => {
    ...
    backend.delete_script(data).then(r => {
      if (r.error == 'False') {
        props.reload();
        props.delete_script_reload(item.name, 1);
      } else {
        props.delete_script_reload(r.message, 3);
      }
    });
  };
    ...
    <TouchableOpacity
      ...
      <View style={{flexDirection: 'row'}}>
        <View style={{flexDirection: 'column', flex: 1}}>
          ...
          <View style={fut.ltpsize}>
            <Text style={[fut.chnage, {color: chng > 0 ? 'green' : 'red'}]}>
...
export default connect(MapStateToProps)(FutCard);
